...
def install(
    ...
    if not execute_and_process_command(f'mkfs.vfat -F32 /dev/{disk_data.boot}', 'Formatting an EFI partition to FAT32'):
        return False

    # Formatting the / partition to the selected file system
    if int_data.file_system == "1":
        if not execute_and_process_command(f'mkfs.f2fs -f /dev/{disk_data.system}', 'Formatting the / partition to F2FS file system'):
            return False
    elif int_data.file_system == "2":
        if not execute_and_process_command(f'mkfs.ext4 -f /dev/{disk_data.system}', 'Formatting the / partition to EXT4 file system'):
            return False
    elif int_data.file_system == "3":
        if not execute_and_process_command(f'mkfs.btrfs -f /dev/{disk_data.system}', 'Formatting the / partition to BTRFS file system'):
            return False
    else:
        return False
    ...
    if not execute_and_process_command(f'mkswap -f /dev/{disk_data.swap}', 'Creating a swap partition'):
        return False
    if not execute_and_process_command(f'swapon -f /dev/{disk_data.swap}', 'Activating a swap partition'):
        return False

    # Mounting the EFI partition and / into the /mnt directory
    if not execute_and_process_command('mkdir -p /mnt/', 'Creating a folder /mnt'):
        return False
    if not execute_and_process_command(f'mount /dev/{disk_data.system} /mnt', 'Mounting the / partition into the /mnt directory'):
        return False
    if not execute_and_process_command('mkdir -p /mnt/boot/efi', 'Creating a folder /mnt/boot/efi'):
        return False
    if not execute_and_process_command(f'mount /dev/{disk_data.boot} /mnt/boot/efi', 'Mounting the EFI partition into the /mnt/boot/efi directory'):
        return False

    # Activating multilib repositories (To install the system)
    if not execute_and_process_command('sed -i "/\[multilib\]/,/Include/"\'s/^#//\' /etc/pacman.conf', 'Activating multilib repositories (To install the system)'):
        return False
    ...
    if not execute_and_process_command(f'{package_data.kernel}', 'Installing the kernel'):
        return False
    if not execute_and_process_command('genfstab -U /mnt >> /mnt/etc/fstab', 'Generating fstab file'):
        return False
    ...
    if not execute_and_process_command(f'arch-chroot /mnt /bin/bash -c "ln -sf /usr/share/zoneinfo/{user_data.timezone} /etc/localtime"', 'Setting the time zone'):
        return False
    if not execute_and_process_command('arch-chroot /mnt /bin/bash -c "hwclock --systohc"', 'Synchronizing time'):
        return False

    # Setting up the locale and generating local data
    if not execute_and_process_command('arch-chroot /mnt /bin/bash -c "sed -i s/#en_US.UTF-8/en_US.UTF-8/g /etc/locale.gen"', 'Uncommenting English language'):
        return False
    ...
    if not execute_and_process_command('arch-chroot /mnt /bin/bash -c "echo KEYMAP=en > /etc/vconsole.conf"', 'Setting the keyboard layout'):
        return False
    ...
    if not execute_and_process_command('arch-chroot /mnt /bin/bash -c "sed -i s/#VerbosePkgLists/VerbosePkgLists/g /etc/pacman.conf"', 'Activating detailed package list (For system)'):
        return False
    if not execute_and_process_command('sed -i "/\\[multilib\\]/,/Include/"\'s/^#//\' /mnt/etc/pacman.conf', 'Activating installing multiple downloads (For system)'):
        return False
    ...
    if not execute_and_process_command('arch-chroot /mnt /bin/bash -c "pacman -Syy grub efibootmgr --noconfirm"', 'Installing the necessary utilities for EFI'):
        return False
    ...
    if not execute_and_process_command('arch-chroot /mnt /bin/bash -c "exit"', 'Leaving a arch-chroot environment'):
        return False
